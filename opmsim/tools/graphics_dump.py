...
class PupilPlotObject():
    ...
    def plot_autoscale(self, fig, rotate_90=False, use_circle_path=False,
            ...
            format_ = '%.02f'
            if (np.max(data_x)-np.min(data_x)) < 1e-3:
                format_ = '%.03f'
                if (np.max(data_x)-np.min(data_x)) < 1e-4:
                    print("change scale")
                    min_for_scale = np.min(data_x)-1e-2
                    max_for_scale = np.max(data_x)+1e-2
                    cb_update = False
            ...
            cb = fig.colorbar(pc1, ax=ax, fraction=0.04, pad=pad, ticks=cbar_ticks, format=format_,location="bottom")
            # have to do this every time? makes no sense
            if cb_update:
            ...
            if (np.max(data_y)-np.min(data_y)) < 1e-3:
                format_ = '%.03f'
                if (np.max(data_y)-np.min(data_y)) < 1e-4:
                    print("change scale")
                    min_for_scale = np.min(data_y)-1e-2
                    max_for_scale = np.max(data_y)+1e-2
                    cb_update = False
            ...
            cb2 = fig.colorbar(pc2, ax=ax2, fraction=0.04, pad=pad, ticks=cbar_ticks, format=format_,location="bottom")
            # have to do this every time? makes no sense
            if cb_update:
                tick_locator = ticker.MaxNLocator(nbins=4, prune='both')
                cb2.locator = tick_locator
            ...
            if (np.max(data_total)-np.min(data_total)) < 1e-3:
                format_ = '%.03f'
                if (np.max(data_total)-np.min(data_total)) < 1e-4:
                    print("change scale")
                    min_for_scale = np.min(data_total)-1e-2
                    max_for_scale = np.max(data_total)+1e-2
                    print("min_for_scale", min_for_scale)
                    print(max_for_scale)
            ...
            cb3 = fig.colorbar(pc3, ax=ax3, fraction=0.04, pad=pad, ticks=cbar_ticks, format=format_,location="bottom")
            if cb_update:
                tick_locator = ticker.MaxNLocator(nbins=4, prune='both')
                cb3.locator = tick_locator
                cb3.update_ticks()

            data_total_af = data_total
            print("CHANGE IN DATA????", np.sum(data_total_af-data_total_bf))
