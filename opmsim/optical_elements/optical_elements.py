...
from opmsim.tools import graphics
from opmsim import matrices
...
class SineLens(BaseElement):
    ...
    def __init__(self, NA, focal_length, 
        ...
        self.y_axis_rotation = y_axis_rotation
    ...
    def compute_transmission(self, rays):
        ...
        ps_project = matrices.ps_projection_matrix(
            parallel, senkrecht, np.squeeze(rays.k_vec))
        ps_project_inv = matrices.ps_projection_matrix(
        ...
        if self.ar_coating_ri is not None:
            ...
            mat_t, theta_i_after = matrices.thin_film_fresnel_matrix(
        ...
        # mat_t = np.identity(3)
        e_before = rays.transfer_matrix @ rays.e_field
        ...
        intensity_per_dipole_vector = np.mean(I_before, axis=0)
        ...
        intensity_per_dipole_vector = np.mean(I_after, axis=0)
        I_per_dipole = np.sum(intensity_per_dipole_vector, axis=1)
    ...
    def trace_rays(self, rays):
        ...
        if self.y_axis_rotation != 0:
            print("rotating rays by", self.y_axis_rotation)
            # print("before rotating theta", rays.theta)
            # print("before rotating rho" , rays.rho)
            self.rotate_rays_y(rays)  # for angled lenses i.e. O3
            if self.update_history: rays.update_history(("x_axis rotation %.2f rads before lens" % self.y_axis_rotation))
        ...
        if all(abs(old_theta[np.invert(rays.escaped)]) < 1e-10):  # soft inequality for theta = 0
            ...
            rays.k_vec = \
                matrices.refraction_meridional_tensor(-lens_theta) @ rays.k_vec
            rays.transfer_matrix = \
                matrices.refraction_meridional_tensor(-lens_theta) @ rays.transfer_matrix
    ...
    def rotate_rays_y(self, rays):
        ...
        # get rotated k_vec
        rotated_k_vec = matrices.rotate_rays_y(y_rot) @ rays.k_vec
        rays.transfer_matrix = matrices.rotate_rays_y(y_rot) @ rays.transfer_matrix
...
class LinearPolariser():
    ...
    def trace_rays(self, rays, update_history=False):
        ...
        if rays.isMeridional:  # put back into non meridional basis
            rays.meridional_transform(inverse=True)
        # rays.k_vec = matrices.polariser(self.psi) @ rays.k_vec
        rays.transfer_matrix = matrices.polariser(self.psi) @ rays.transfer_matrix
        return rays
...
class DiagonalMatrix():
    ...
    def trace_rays(self, rays, update_history=False):
        ...
        if rays.isMeridional:  # put back into non meridional basis
            rays.meridional_transform(inverse=True)
        rays.transfer_matrix = \
            matrices.diagonal(self.value) @ rays.transfer_matrix
        return rays
...
class WavePlate():
    ...
    def trace_rays(self, rays, update_history=False):
        ...
        if rays.isMeridional:  # put back into non meridional basis
            rays.meridional_transform(inverse=True)
        # rays.k_vec = matrices.wave_plate(self.psi, self.delta) @ rays.k_vec
        # E_before = rays.transfer_matrix @ rays.e_field
        #rays_, e_field_ = 
        if self.plot_debug:
        ...
        rays.transfer_matrix = \
            matrices.wave_plate(self.psi, self.delta) @ rays.transfer_matrix
        if self.plot_debug:
            ...
            e_field = rays.transfer_matrix @ rays.e_field
            data_x =  np.real(e_field[0,:,0]*np.conj(e_field[0,:,0]))
            data_y =  np.real(e_field[0,:,1]*np.conj(e_field[0,:,1]))
...
class FlatMirror():
    ...
    def trace_rays(self, rays):
        ...
        if self.plot_debug:
            # starting field
            initial_e_field_4d = (rays.transfer_matrix @ rays.e_field)#.squeeze()
            initial_e_field = initial_e_field_4d[0,:,:,0]
        ...
        if self.plot_debug:
            ...
            data_x =  np.real(initial_e_field[:,0]*np.conj(initial_e_field[:,0]))
            data_y =  np.real(initial_e_field[:,1]*np.conj(initial_e_field[:,1]))
        ...
        ps_project = matrices.ps_projection_matrix(parallel, senkrecht, np.squeeze(rays.k_vec))
        ps_project_inv2 = matrices.ps_projection_matrix(parallel, senkrecht, np.squeeze(rays.k_vec), inverse=False)
        ...
        if self.perfect_mirror:
            ...
            if self.n_film_data is None:
                ...
                M_fresnel = matrices.single_suface_fresnel_matrix(
                    theta_i, self.n_metal_data, rays.lda)
            else:
                print("USING AIRY SUM MIRROR")
                M_fresnel = matrices.thin_film_fresnel_matrix(
        ...
        reflection_mat = matrices.reflection_cartesian_matrix(N.squeeze())
        ...
        if self.plot_debug:
            ...
            E_in = initial_e_field
            E_ps_proj = ps_project @ initial_e_field_4d
...
class PerfectRightAngleMirror(BaseElement):
