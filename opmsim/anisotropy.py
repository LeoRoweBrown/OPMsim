...
def calculate_anisotropy(detector, polariser_phi=None):
    ...
    if polariser_phi is None:
        ...
        I_p = detector.Ix_integral * np.cos(polariser_phi) + detector.Iy_integral * np.sin(polariser_phi)
        I_s = detector.Ix_integral * np.sin(polariser_phi) + detector.Iy_integral * np.cos(polariser_phi)
...
def theoretical_anisotropy(NA, dipole_orientation, I_p_yaxis=True, return_intensities=False):
    ...
    x = [np.sin(alpha), np.cos(alpha) * np.cos(phi), np.cos(alpha) * np.sin(phi)]
